        ...
        switch (entry.path) {
          ...
          default:
            if ((match = entry.path.match(/xl\/(worksheets\/sheet(\d+)\.xml)/))) {
              ...
              if (!self.options.sheetNumbers || self.options.sheetNumbers.includes(parseInt(sheetNo))) { 
                if (self.parsedWorkBookInfo === false ||
                  ...
                  const { name } = Tmp.fileSync(self.options.tmpOptions)
                  ...
                  if(self.options.enableCompression){
                    entry.pipe(createGzip()).pipe(stream)
                  }else{
                    entry.pipe(stream)
                  }
                } else {
      ...
      .on('close', function (entry) {
        if (self.waitingWorkSheets.length > 0) {
          ...
          var processBooks = function () {
            ...
            var workSheetStream
            if(self.options.enableCompression){              
              workSheetStream = Fs.createReadStream(sheetInfo.path).pipe(createGunzip())
            }else{
              workSheetStream = Fs.createReadStream(sheetInfo.path)
            }
